...
// Simple bcrypt-like hashing for seed (since AuthService may not be available in Docker)
async function hashPassword(password: string): Promise<string> {
  // Use SHA256 hash for seed fallback
...
async function main() {
  ...
  try {
    ...
    // Create 10 students for each group (Total 40) - Only if no students exist
    const studentCount = await prisma.student.count()
    if (studentCount > 0) {
      console.log(`⏭️ Skipping student creation (${studentCount} students already exist)`)
    } else {
